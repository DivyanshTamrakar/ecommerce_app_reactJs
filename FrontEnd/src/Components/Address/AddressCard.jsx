...
function AddressCard({
  ...
  return (
    <div className="AddressCard" onClick={() => setclickvalue(_id)}>
      <div>Address {index + 1}</div>
      <span className="AddressText fontWeight">{name}</span>
      ...
      {clickvalue === _id && (
        <span>
          <Link to="/ordersummary" className="textDecorationNone">
            <button className="btn deliver"> Deliver to this address</button>
          </Link>
        </span>
      )}
    </div>
